...
export const BudgetSection = () => {
  ...
  const renderCurrency = (valueInPEN: number, usdClassName = "text-2xl") => {
    const exchangeRate = budget.exchange_rate
    ...
    return (
      <span className="flex flex-col  items-center xl:inline-flex">
        <span>{formattedPEN}</span>
        <span className={`${usdClassName} opacity-70 sm:inline-flex px-2`}>({formattedUSD})</span>
      </span>
    )
  }
